...
class IngestDocumentsUseCase:
    def __init__(self, ocr_service, chunker, vector_db_repo):
    ...
    def execute(self, pdf_path):
        dl_doc = self.ocr_service.convert_document(pdf_path)
        chunks, metadatas = self.chunker.chunk_docling(dl_doc)
        self.vector_db_repo.add_chunks(chunks, metadatas)